...
class HomeSecurityAssistantPanel extends HTMLElement {
  constructor() {
    ...
    this._pendingExternalTarget  = null;
  ...
  _unregisterBeforeUnload() {
    ...
    this._unregisterNavInterceptor();
  }

  _registerNavInterceptor() {
    if (this._origPushState) return;
    var self = this;
    this._origPushState = history.pushState.bind(history);
    // Patch history.pushState — HA calls this for every SPA navigation.
    this._navInterceptHandler = function(state, title, url) {
      if (!self._settingsDirty || self._view !== 'settings') {
        self._origPushState(state, title, url);
        return;
      }
      // Block navigation and show the unsaved-changes modal.
      self._pendingExternalTarget = String(url || '');
      self._pendingView = '_external_';
      self._render();
    };
    history.pushState = this._navInterceptHandler;
  }

  _unregisterNavInterceptor() {
    if (!this._origPushState) return;
  ...
  _onClick(e) {
    ...
    if (e.target.closest('[data-unsaved-leave]')) {
      ...
      var extTarget = this._pendingExternalTarget;
      this._pendingView = null;
      this._pendingExternalTarget = null;
      this._settingsDraft = {};
      this._settingsDirty = false;
      var origPush = this._origPushState;
      this._unregisterBeforeUnload();
      if (dest === '_external_' && extTarget) {
        if (origPush) {
          origPush(null, '', extTarget);
    ...
    if (!topIPs.length) {
      ...
      ipsSection = '<table class="data-table" style="width:100%;margin-top:8px"><thead><tr>' +
        ...
        topIPs.map(function(e, i) {
          ...
          var cc = (e.country || '').toLowerCase();
          var countryTitle = e.country_name || e.country || '';
          var flagImg = cc.length === 2 ? '<img src="https://flagcdn.com/16x12/' + cc + '.png" width="16" height="12" alt="' + self._esc(e.country || '') + '" title="' + self._esc(countryTitle) + '" style="vertical-align:middle;border:none;border-radius:1px;margin-right:4px">' : '';
          ...
          return '<tr><td style="color:var(--muted)">' + (i + 1) + '</td>' +
            ...
            '<td style="white-space:nowrap">' + flagImg + self._esc(countryTitle) + '</td>' +
    ...
    if (!topC.length) {
      ...
      countriesSection = '<table class="data-table" style="width:100%;margin-top:8px"><thead><tr>' +
        ...
        topC.map(function(c, i) {
          ...
          var countryTitle = c.country_name || c.country || '';
          var flagImg = cc.length === 2 ? '<img src="https://flagcdn.com/16x12/' + cc + '.png" width="16" height="12" alt="' + self._esc(c.country || '') + '" title="' + self._esc(countryTitle) + '" style="vertical-align:middle;border:none;border-radius:1px">' : '\u2014';
          return '<tr><td style="color:var(--muted)">' + (i + 1) + '</td>' +
            '<td style="text-align:center">' + flagImg + '</td>' +
            '<td>' + self._esc(countryTitle || '\u2014') + '</td>' +
    ...
    if (!_extDevTotal) {
      ...
      extIpDeviationSection = '<table class="data-table" style="width:100%;margin-top:8px"><thead><tr>' +
        ...
        _extDevItems.map(function(x, i) {
          ...
          var ccDev = (enr && (enr.country || '')).toLowerCase();
          var countryTitleDev = (enr && (enr.country_name || enr.country)) || '';
          var flagImgDev = ccDev.length === 2 ? '<img src="https://flagcdn.com/16x12/' + ccDev + '.png" width="16" height="12" alt="' + self._esc((enr && enr.country) || '') + '" title="' + self._esc(countryTitleDev) + '" style="vertical-align:middle;border:none;border-radius:1px;margin-right:4px">' : '';
          return '<tr><td style="color:var(--muted)">' + (i + 1) + '</td>' +
            '<td><span class="ip">' + self._esc(x.ip) + '</span></td>' +
            '<td style="max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + self._esc(org) + '</td>' +
            '<td style="white-space:nowrap">' + flagImgDev + self._esc(countryTitleDev) + '</td>' +
            '<td style="text-align:right">' + x.count + '</td></tr>';
        }).join('') +
        '</tbody></table>';
    }
  ...
  _extPreparedList() {
    ...
    extIPs.sort(function(a, b) {
      ...
      if (sortKey === 'ip') {
        ...
        // Higher number = more dangerous; descending (▼) puts malicious first
        var rOrder = { malicious: 3, suspicious: 2, clean: 1, '': 0 };
        va = rOrder[a.rating || (a.blacklisted ? 'malicious' : '')] !== undefined ? rOrder[a.rating || (a.blacklisted ? 'malicious' : '')] : 0;
        vb = rOrder[b.rating || (b.blacklisted ? 'malicious' : '')] !== undefined ? rOrder[b.rating || (b.blacklisted ? 'malicious' : '')] : 0;
        return (va - vb) * sortDir;
      } else if (sortKey === 'vt') {
  ...
  _extRows() {
    ...
    return extIPs.map(function(e) {
      ...
      return '<tr>' +
        ...
        '<td style="font-size:11px">' + (cc.length === 2 ? '<img src="https://flagcdn.com/16x12/' + cc + '.png" width="16" height="12" alt="' + self._esc(e.country || '') + '" title="' + self._esc(countryTitle) + '" style="vertical-align:middle;border:none;border-radius:1px">' : '<span class="dim">\u2014</span>') + '</td>' +
  ...
  _viewSettings() {
    ...
    return '<div>' +
      ...
      '<div id="hsa-settings-msg">' + msgHtml + '</div>' +
      '<div id="hsa-settings-dirty-banner">' + (this._settingsDirty ? '<div style="background:rgba(255,206,84,.1);border:1px solid rgba(255,206,84,.45);border-radius:8px;padding:8px 12px;margin-bottom:14px;color:#ffce54;font-size:11px">Unsaved changes in this view are kept locally until you click Save settings or Reload from disk.</div>' : '') + '</div>' +
      fieldsHtml +
      ...
      '</div>' +
      '<div style="margin-top:24px;padding-top:14px;border-top:1px solid var(--border);text-align:center;font-size:11px;color:var(--muted)">' +
        '<a href="https://domotic.monster" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:none">\u00a9 2026 domotic.monster</a>' +
      '</div>' +
    '</div>';
  }
  ...
  _onSettingsSave() {
    ...
    this._hass.callApi('POST', 'homesec/settings/save', payload).then(function() {
      ...
      // Update local config in-place so the form stays populated without re-fetching.
      if (self._settingsData && self._settingsData.config) {
        Object.assign(self._settingsData.config, payload);
      }
      // Targeted DOM update: show success message without rebuilding the entire form.
      var root = self.shadowRoot;
      var msgWrap = root && root.getElementById('hsa-settings-msg');
      var dirtyWrap = root && root.getElementById('hsa-settings-dirty-banner');
      if (msgWrap && dirtyWrap) {
        msgWrap.innerHTML = '<div style="background:rgba(0,0,0,.25);border:1px solid var(--success);border-radius:8px;padding:10px 14px;margin-bottom:14px;color:var(--success);font-size:12px">Settings saved. The integration will reload in a moment to apply changes.</div>';
        dirtyWrap.innerHTML = '';
      } else {
        self._render();
      }
