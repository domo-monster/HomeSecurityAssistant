...
class HomeSecurityAssistantPanel extends HTMLElement {
  ...
  _ipDetail(d, internalSources) {
    ...
    // ── Shodan section ─────────────────────────────────────────────────────
    var hasShodan = (d.shodan_services && d.shodan_services.length) ||
      (d.shodan_ports && d.shodan_ports.length) ||
      (d.shodan_domains && d.shodan_domains.length) ||
      (d.shodan_technologies && d.shodan_technologies.length) ||
      (d.shodan_vulns && d.shodan_vulns.length) ||
      (d.shodan_tags && d.shodan_tags.length) ||
      d.shodan_org || d.shodan_os;
    var shodanSection = '';
    if (hasShodan) {
      var shodanRows = '';
      // Open ports with service/product/version
      if ((d.shodan_services && d.shodan_services.length) || (d.shodan_ports && d.shodan_ports.length)) {
        var portChips;
        if (d.shodan_services && d.shodan_services.length) {
          portChips = d.shodan_services.map(function(s) {
            var label = s.port + '/' + s.transport;
            if (s.product) label += ' \u00b7 ' + s.product;
            if (s.version) label += ' ' + s.version;
            return '<span class="chip">' + self._esc(label) + '</span>';
          }).join(' ');
        } else {
          portChips = d.shodan_ports.map(function(p) { return '<span class="chip">' + p + '</span>'; }).join(' ');
        }
        shodanRows += '<div class="shodan-row"><span class="shodan-label">Open Ports</span><span class="shodan-val">' + portChips + '</span></div>';
      }
      // Domains
      if (d.shodan_domains && d.shodan_domains.length) {
        var domainChips = d.shodan_domains.map(function(dom) { return '<span class="chip">' + self._esc(dom) + '</span>'; }).join(' ');
        shodanRows += '<div class="shodan-row"><span class="shodan-label">Domains</span><span class="shodan-val">' + domainChips + '</span></div>';
      }
      // Web technologies
      if (d.shodan_technologies && d.shodan_technologies.length) {
        var techChips = d.shodan_technologies.map(function(t) { return '<span class="chip">' + self._esc(t) + '</span>'; }).join(' ');
        shodanRows += '<div class="shodan-row"><span class="shodan-label">Technologies</span><span class="shodan-val">' + techChips + '</span></div>';
      }
      // CVEs
      if (d.shodan_vulns && d.shodan_vulns.length) {
        var vulnBadges = d.shodan_vulns.map(function(v) { return '<span class="badge badge-critical" style="font-size:9px">' + self._esc(v) + '</span>'; }).join(' ');
        shodanRows += '<div class="shodan-row"><span class="shodan-label">CVEs</span><span class="shodan-val">' + vulnBadges + '</span></div>';
      }
      // Tags
      if (d.shodan_tags && d.shodan_tags.length) {
        var tagChips = d.shodan_tags.map(function(t) { return '<span class="chip">' + self._esc(t) + '</span>'; }).join(' ');
        shodanRows += '<div class="shodan-row"><span class="shodan-label">Tags</span><span class="shodan-val">' + tagChips + '</span></div>';
      }
      // Org / OS
      if (d.shodan_org || d.shodan_os) {
        var orgOs = [d.shodan_org && self._esc(d.shodan_org), d.shodan_os && 'OS: ' + self._esc(d.shodan_os)].filter(Boolean).join(' \u00b7 ');
        shodanRows += '<div class="shodan-row"><span class="shodan-label">Org / OS</span><span class="shodan-val">' + orgOs + '</span></div>';
      }
      shodanSection = '<div class="shodan-section"><div class="shodan-header">Shodan</div>' + shodanRows + '</div>';
    }
